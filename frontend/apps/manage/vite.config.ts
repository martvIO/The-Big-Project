...
// breaks the same way.
//
// A leading "^" is what makes Vite read the key as a RegExp. The ten names are
// exactly the second path segments the backend's /manage routers declare, and
// Backend/tests/test_spa_serving.py derives that set from the live route table
// and asserts this line matches it — an eleventh router added without touching
// this file fails there rather than silently 404ing in dev only.
const MANAGE_API =
  "^/manage/(appointment-types|auth|availability|bookings|dashboard|dresses|settings|slots|staff|terms)";
