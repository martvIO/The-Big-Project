"""F13's rows in the permanent cross-tenant isolation suite: customers (a
bride's phone and name under one boutique must be invisible to every other) and
bookings (a foreign tenant reads nothing, and its bookings never contend for
another tenant's seats — the slot-seat unique index is scoped per tenant).
