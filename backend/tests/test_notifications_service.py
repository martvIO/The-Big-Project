...
def _service(
    ...
    sender: FakeSmsSender | UnconfiguredSmsSender | ExplodingSmsSender | None = None,
    *,
    phone_limiter: FixedWindowRateLimiter | None = None,
    dev_code: str | None = None,
) -> tuple[OtpService, FakeSmsSender | UnconfiguredSmsSender | ExplodingSmsSender, WallClockStub]:
...
async def test_unconfigured_sender_records_failure_and_503_shape(app_role_url: str) -> None:
    engine, factory = _factory(app_role_url)
    tenant_id = uuid.uuid4()
    ...
    try:
        ...
        assert rows[0].error == "not configured"
    finally:
        await engine.dispose()
...
async def test_provider_explosion_is_contained(app_role_url: str) -> None:
    ...
    try:
        ...
        async with tenant_session(factory, tenant_id) as session:
            rows = await MessageLogRepository().list_by_phone(session, tenant_id, phone=NORMALIZED)
        assert [row.status for row in rows] == [MessageStatus.FAILED.value]
        assert rows[0].error is not None and "provider says no" in rows[0].error
    finally:
        await engine.dispose()
