...
**Provisioning tenants (platform console)**: onboarding runs in the web console at `https://admin.{BASE_DOMAIN}/platform` — a third workspace app (`Frontend/apps/platform`) in front of the same audited `ProvisioningService` the CLI used to call. Provision (slug, name, owner email, initial owner password), suspend, list and reset an owner's password; every state change writes to `platform_audit_log` — a **platform-scoped** table (column `target_tenant_id`, deliberately not `tenant_id`, so it stays cross-tenant-readable and isn't caught by the forced-RLS metadata scan) — with the **authenticated operator's email** in `operator`, where the CLI wrote `$USER`. The console lives on the RESERVED `admin` label and is fenced BOTH ways: nothing but `/platform*` answers there, and `/platform*` answers on no tenant host. Sessions are 4h, host-only, under their own cookie name.

**Operator CLI (`python -m app.cli`)** is now the MAINTENANCE surface only. F25 deleted `provision` / `suspend` / `list` / `reset-password` at parity — two doors to one audited operation is how they drift. What is left: `create-operator` / `deactivate-operator` (the console's bootstrap — **no HTTP route mints an operator**, so the console's own compromise cannot make a second one; password read from stdin/getpass, never argv), the one-time `backfill-booking-links` (**run it once at the F16 deploy**: the booking flow shipped before any SMS existed, so already-confirmed future bookings have no manage link and no reminder — the command mints both, is safe to re-run, and reads no password so a deploy hook cannot hang on a prompt), and `retention` (F20's `--armed` rehearsal). `--operator` is REQUIRED on `retention` and on both operator commands.
...
**Booking comms (F16)**: a confirmed booking sends an immediate Hebrew SMS carrying the date, the time and a **tokenized manage link** (`{slug}.{base}/b/{token}`), and a reminder follows — at `starts_at − 24h` when the booking is that far out, immediately when it is 2–24h out, and not at all under 2h, where the confirmation is seconds old. Only the token's sha256 is stored on `bookings`; the raw value lives in the SMS body and, while a reminder is still pending, on that `scheduled_messages` row, so the worker can reproduce the same link days later (`message_log` stores the body with the token masked). The link opens a public page where the customer confirms attendance or cancels within her **accepted** policy version — actions expire at `starts_at`, the page stays readable after it — and a cancel frees the seat structurally, because both partial unique indexes exclude `status = 'cancelled'`. `make worker` runs the poller: every `worker_poll_interval_seconds` it enumerates active tenants and claims due rows with `FOR UPDATE SKIP LOCKED` under one tenant context each, so a second replica is safe and a window missed during a deploy self-heals on the next tick. **With no SMS provider configured nothing here is an outage**: due reminders stay pending and flush on the first tick after an adapter lands, confirmations are skipped with one log line, and the booking is a 201 either way.
...
Browse **`http://{slug}.localtest.me:5173/manage/`** for the console and **`:5174`** for the storefront (e.g. `http://bella.localtest.me:5174` after provisioning slug `bella`) — not plain `localhost`. The console's `/manage/` suffix is how it is addressed in dev too: `apps/manage` builds and serves with `base: "/manage/"` so its assets live under the same path the API mounts them at in production. Vite 302-redirects a bare `:5173/` to `/manage/`, so browsing the root still works; any *other* off-base path is a 404. Each Vite dev server proxies to `http://localhost:8000` with `changeOrigin: false`, so the original `{slug}.localtest.me` Host header reaches the backend — the storefront by prefix (`/storefront` plus `/health`), the console by an explicit regex naming the API's nine second segments (`^/manage/(appointment-types|auth|…)` plus `/health`). It cannot use a bare `/manage` prefix: `base: "/manage/"` puts the console's own shell, assets and HMR requests under that prefix too, and Vite's proxy middleware runs *before* its static and transform middlewares, so a prefix key would forward the app itself to the backend and it would never load. A backend test derives that segment list from the live route table and fails if the two drift. tenant resolution and the host-only session cookie work exactly as in production. `allowedHosts: [".localtest.me"]` in each app's `vite.config.ts` is what lets Vite accept the subdomain Host at all — without it the proxy alone is not enough. Both apps are same-origin in production and proxied in dev; **CORS must never be added for either**.
