...
> **Addendum (design gate, 2026-07-29)**: the confirmation SMS **drops its location/maps line** — design-critic segment arithmetic showed the one body carrying both a free-text location and the manage link cannot honestly clear `CONFIRMATION_MAX_SEGMENTS = 3` (the location is unbounded tenant text). The manage page's ContactPanel carries maps/waze instead, so the link she gets leads to the location anyway. The `maps_url`-null fallback rule now applies to the **page**, not the SMS body, and the segment fixture gains a ≤25-char boutique-name bound.
...
- Delivery receipts/DLR ingestion; retry/requeue of failed sends (evidence row + the resend remedy F15 ships is the v1 answer; the *signal* — a failed-send indicator — is the provider-go-live feature's).
...
3. **Kosher phones never receive any SMS** — confirmation/reminder can fail permanently for a legitimate customer; F15 ships the remedy surface (resend) and not the signal, so the only trace of a permanent failure remains a `failed` message_log row that nothing renders — and that trace exists **only once a real provider is configured**: in the unconfigured state F11 deliberately raises before any insert, so pre-provider degradation is evidence-free (one app-log warning; the pending `scheduled_messages` row is the reminder's only trace). Accepted until a provider is live and something renders the failure. *Owner: team. Trigger: provider go-live.*
