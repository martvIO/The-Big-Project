...
> **Amendment (review phase, 2026-07-31) — two money bugs in the `create_session` bullet above.** Both were written into the spec and both shipped from it; the bullet is wrong as printed and the code is right.
>
> **1. `custom_price` is a TOP-LEVEL checkout attribute, not a member of `checkout_data`.** The bullet lists it in both places in one sentence ("`attributes.checkout_data.custom_price = amount_agorot` … `attributes.custom_price`…"), and the nested spelling is the one that was built. It is a sibling of `checkout_data` / `product_options` / `checkout_options`. Two independent official SDKs settle it: the JS SDK builds `attributes = { customPrice, expiresAt, preview, testMode, productOptions, checkoutOptions, checkoutData }` and its `CheckoutData` type carries no price field at all; the Go SDK's `CheckoutAttributes` has `CustomPrice` beside `CheckoutData`, whose struct is only Email/Name/BillingAddress/TaxNumber/DiscountCode/Custom. Consequence of the nested form: Lemon Squeezy silently drops the unrecognised key, answers 201, and mints the checkout at the **placeholder variant's own price** — the bride is charged the wrong amount, and F17's D14 amount assertion then refuses to settle, so she pays the wrong sum *and* loses the seat.
>
> **The durable fix is not the hoist — it is the read-back.** We are forbidden from calling the live API (Risk 2), so asserting the request shape can only ever prove we sent what we *meant* to, never that LS understood it, and a silently-dropped attribute is invisible to every test we are allowed to write. `create_session` therefore requires the provider to **echo `custom_price` back equal to `amount_agorot`**, right beside the existing `test_mode` check, and refuses otherwise — missing and null included. That converts the whole class of field-placement bug into a loud failure on the first real call, before any money moves. The same lens is applied to identity: a `checkout_data.custom` echo that is **present and disagrees** with the reference is refused too, while an absent one is tolerated, because whether a create response populates `checkout_data` could not be verified against the live API and refusing on absence would reject every legitimate checkout.
>
> Both guards raise `GatewayCredentialsRejectedError` (**400**), not `GatewayUnavailableError`, for the reason the `test_mode` guard already gives: the wrongly-priced checkout **already exists at the provider and is payable**, so a 503's "temporarily unavailable" would invite a retry that mints another one — and the condition is not transient anyway, being either our bug or an API change.
>
> **2. `create_session` returns `provider_session_id = reference`, NOT `data.id`.** The bullet's `PaymentSession(provider_session_id=data.id, …)` is the money bug F19's D3a records. The port defines one identifier space — `open_deposit` stores what `create_session` returns and `settle_from_webhook` looks that row up by what `verify_webhook` returns — and an LS *order* carries no checkout id, while `checkout_data.custom` is fixed before the checkout id exists. So the reference is the only key present on both halves; returning `data.id` meant a verified, genuinely-paid webhook matched no row (charge taken, payment left `pending`, seat freed by the sweeper). The LS checkout id is still required to be present in the response and is logged at INFO so an operator can find the checkout in the dashboard — it simply does not identify the session to us. Pinned by `test_the_two_halves_agree_on_provider_session_id` and by an end-to-end composition test.
