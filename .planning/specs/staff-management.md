...
**And a password write revokes the target's other sessions** (added by the 2026-07-30 review, which showed the paragraph above was claiming a containment the code did not provide). `resolve_session` re-reads `staff_users`, which is why *deactivation* bites instantly with no sweep — but it never consults `password_hash`, so without an explicit revoke a stolen cookie outlives the password change meant to contain it for the whole `session_ttl_seconds` (12 h). Inside that window `POST /manage/staff` — which F51 is what first puts behind a session cookie — mints a *standalone second owner* whose credential survives the TTL, every later password and every logout: the permanent takeover the `current_password` check is supposed to prevent, reached in one request. `SessionsRepository.revoke_for_staff_user` runs in the same transaction under the same lock, excluding the acting cookie so the owner is not signed out of the tab she just used. A rename or a role change revokes nothing — those are not credential events. This does **not** reopen the do-not-sweep-on-deactivation ruling, which is about a different write with a seam that already works.
...
9. **A stolen owner session can still reset every *other* staffer's password without knowing it** — `current_password` is required only on the self path (D4), by design, since an owner does not know her staff's passwords. The compromise this leaves is "an attacker with a live owner session can lock out every staffer but the owner" — and since the 2026-07-30 review it also signs each of them out of every device, because a password write now revokes the target's sessions. Still strictly smaller than the console access she already has, and still bounded: the owner's own password change now ends the attacker's session too. *Owner: user. Trigger: the F21 security audit.*
10. **The window between theft and the owner noticing is still fatal.** Session revocation closes the compromise the moment the owner changes her password; it cannot undo what the attacker did first. An attacker who creates a second owner *before* the password change holds a credential no F51 control can reach — the remedy is the operator CLI. Nothing in this product can shrink that window without a notification channel (Risk 2's blocker). *Owner: user. Trigger: the F21 security audit, or transactional email arriving.*
...
## 2026-07-30 adversarial review — applied and rejected

Three lenses returned nine findings against the F51 diff; two pairs were duplicates, so seven distinct items. Each was re-verified against the code before anything was changed.

### Applied

- **The create form had no email check, so a routine typo rendered pydantic's English into the RTL console.** `<Input type="email">` passes `dana@bella` — WHATWG makes the dot in the domain optional — and `EmailStr` then answers *«value is not a valid email address: The part after the @-sign is not valid…»*, which `setCreateError(message(error))` renders verbatim in a `role="alert"`. `VALIDATION_ERROR` is not in `MAPPED_CODES`, so it falls through to the server's own text. This falsified two comments that assert the opposite in as many words — `validation.ts`'s *"every other 400 those forms could produce is caught right here"* and `test_frontend_constant_parity.py`'s *"This test is what makes that a checked claim rather than a hope"* — and with them the justification for the field-local `staff.currentPasswordWrong` treatment. **Fix:** `StaffDraft` gains `email: string | null` (the `password` idiom: null on the edit form, where the address is not editable per D5) and `validateStaffDraft` rejects blank and any shape the browser admits but `EmailStr` will not. Mapping `VALIDATION_ERROR` into the Hebrew map was the weaker alternative — it hides the English but leaves the claim false.
- **A password write did not revoke the target's sessions.** See D4 and Risks 9–10 above. `SessionsRepository.revoke_for_staff_user`, called from `StaffService.update` whenever a `password_hash` is written, in the same transaction under the same lock, excluding the acting cookie. One new repository method, no migration, two db tests and two service tests.
- **Both argon2 calls ran inside the advisory lock**, contradicting the rationale `create` states forty lines above for hoisting the same call out (*"there is no reason to hold a pooled connection across it"*). `hash_password` is now computed before `tenant_session`; `verify_password` stays inside because it needs the row's own hash. One KDF run under the lock instead of two.
- **`display_name` accepted a whitespace-only value at the API boundary.** `Field(min_length=1)` admits `"   "`, while all three sibling name validators reject blank — including `boutique/validation.py`, the very field `MAX_DISPLAY_NAME_LENGTH` was borrowed from. A blank name renders as an empty `<bdi>`, findable only by its email. **Fix:** a `DisplayName` annotated type with `StringConstraints(strip_whitespace=True, min_length=1, …)` on both requests. Stripping also collapses `" Dana"` and `"Dana"`, so `update`'s no-op comparison cannot be fooled by an edge space.
- **The deactivate confirm interpolated `display_name` as bare text**, on the one screen whose whole job is naming the right person before a destructive action — while the list row two hundred lines earlier does it correctly. The spec's Accessibility section and the copy deck both require a bare `<bdi>`, and `ProvisioningService.provision` seeds every founding owner with `display_name = owner_email`, so a Latin run inside the Hebrew sentence is the norm here. **Fix:** the `<bdi>` moved into the string in `he.ts`/`ar.ts` (the only key in the deck carrying markup, noted in `copy.md`) and rendered through `<Trans components={{ bdi: <bdi /> }}>`.
- **`StaffSection.tsx` claimed its Hebrew error map was "pinned by `SPEC_ERROR_CODES` … so it cannot silently drift".** It is not: `SPEC_ERROR_CODES` is a Python set checked against a Python module, and no test reads any TypeScript. **Fix:** the comment now says the map is kept by hand and names the failure mode, in `StaffSection.tsx` and in the twin comment in `he.ts`. Building the cross-language scrape was declined — `test_frontend_constant_parity.py` guards *numeric bounds whose values must agree*; encoding "the router's mappable subset" in a test means encoding a judgement call, for a four-entry map, in a repo where the code map is the shorter thing to read. A comment that lies is the defect; a comment that tells the truth fixes it.

### Rejected

**REJECTED — "`StaffMember.created_at` is shipped and typed but read by nothing; drop it."** The observation is correct: `StaffSection.tsx` never renders it, list order is decided server-side by `list_live`, and the only frontend occurrence is a test fixture. The finding itself grades it a YAGNI note rather than a spec violation, and that is exactly why it does not carry: **D6 names `StaffMember` as `{id, email, display_name, role, created_at}`**, so removing it is a spec amendment, not a cleanup. The diff to remove is also larger than the diff to keep — the pydantic model, the TS interface, the router's `_member`, the fixture and the two tests that pin the key set — and `created_at` on a list resource is a standard output whose *absence* would be the thing a reader stops on. Recorded here rather than in D6, so the next reader who goes looking for the renderer finds this paragraph instead.
