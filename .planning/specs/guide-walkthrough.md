...
- **`e2e/` is where focus is measured in this repo, and it says so.** `e2e/manage.spec.ts:26-30` and `e2e/sos.spec.ts:28-40` both open with the same paragraph: jsdom is not a browser, every focus assertion is measured in Chromium, and F57 once shipped a focus test that asserted nothing. `installManageApi` and the payload builders live in `e2e/fixtures/manage.ts` — **there is no login helper there, and there is no login flow to helper-ise (C1).** `installManageApi` authenticates by *not* authenticating (`fixtures/manage.ts:14-17`): `App.tsx` bootstraps on `api.me()` and renders `<LoginForm/>` on a rejection, so fulfilling `GET /manage/auth/me` with a 200 `Staff` body is the whole of «signed in». `grep -n LOGIN_SUBMIT e2e/*.ts` returns four hits — `manage.spec.ts:37`/`:171`, `sos.spec.ts:53`/`:407` — and every one is the **negative** assertion `toHaveCount(0)`, i.e. *"the console did not fall back to login"*. No spec file inlines a login.
...
| 11 | `atelier` | תפירה | ATELIER_ROLES | 3 | The five columns intake→delivered; opening a ticket; the seamstress's own view (`AtelierSection.tsx:1356`'s `mayWork`/`isSeamstress` branch). |
...
**The detector reads EVERY `for_me` alert, dismissed ones included, and it must (C7).** `SosContextValue` (`sos-context.ts:22-36`) exposes `alerts`, `serverNow`, `terminal`, `channelDown` and the four verbs; **`dismissed` is `SosOverlay`'s own `useState` and is not on the context**, so `GuideOverlay` *cannot* subtract it. That is also the correct behaviour: a dismissed alert whose `escalated` flips re-rises the card, and the guide must close for that. Stated because a reviewer will ask why the detector does not mirror `SosOverlay`'s `rising` (which *does* subtract `dismissed`). It must not — and §6b is the test that a *persisting* dismissed alert still lets her reopen.
...
**The title's section label is DERIVED here, not passed in (C3 / P-1).** `GuideOverlayProps` is `{ section: SectionKey }` and the title is ``t("guide.title", { section: t(`nav.${section}`) })``. Every one of the fourteen `NAV` rows has `labelKey === `nav.${key}`` — five resolve through the nested `nav:` object (`apps/manage/src/i18n/he.ts:14-20`, labels at `:15-19`) and nine through i18next's flat-literal fallback, and deck F-6 records that both paths work and **neither may be "fixed" against the other**. Cost: zero props, zero expressions in `App.tsx`, zero new keys, and a nav-label edit can never desynchronise the guide's title. **Reconciling with DL5** (steps are full key literals, never templated): DL5 exists because the console has *no source scanner*, so `HE_F60`'s `startsWith("guide.")` filter is the only thing that can see a step key and a filter can only count literals. `nav.*` is outside that namespace, is indexed by a closed compiler-checked union, and a miss already renders the raw key in the nav row `ConsoleShell` paints on every section. Named test: all fourteen `` `nav.${key}` `` resolve to something other than the key.
...
| **Narrow viewport (375px)** | `Modal`'s `w-[min(28rem,calc(100vw-2rem))]` gives 16px gutters at 375; the panel covers most of the screen and that is correct — this is a modal and there is nothing behind it to read. Steps are one sentence, so no scroll. **The three `size="md"` footer buttons cannot wrap (C4):** `Modal.tsx:63` is `flex justify-end gap-3` with **no `flex-wrap`**, so flex items shrink. They do not need to — deck §2.6 measures it: 343px panel → 295px content box → ≈243px of buttons, ≈50px of slack. At extreme zoom a label wraps to two lines *inside its own button* (`Button.tsx` sets no `whitespace-nowrap`), which is legible and operable; WCAG 2.0 AA has no reflow SC. **Do not add `flex-wrap` to `Modal`** — a fifteen-call-site edit this feature has not earned. | Inline, in normal page flow. Nothing is covered; the form below simply moves down. |
...
**The harness is mandatory, not optional (C2).** `useSos()` **throws** outside `SosProvider` — `lib/sos-context.ts:32-40`, *"Loud rather than inert. A component that reads this outside the provider is one that would render a permanently empty emergency channel."* — and four shipped test files carry the same warning. `GuideOverlay.test.tsx` therefore copies `SosCentre.test.tsx`'s harness verbatim: `vi.mock("../api")` with the nine floor/sos fns, `render(<SosProvider>…</SosProvider>)`, `vi.useFakeTimers({ shouldAdvanceTime: true })`, `getSos.mockResolvedValue(sosPayload([...]))`, and an `advance(ms)` helper wrapping `vi.advanceTimersByTimeAsync` in `act()`. **Declined: `vi.mock`ing `../lib/sos-context`** — one line shorter, and it stubs the exact mechanism §6 exists to measure (the provider's array identity across a poll tick). **The import is `from "../lib/sos-context"`, not `from "../lib/sos"`** — all four shipped consumers import it from there, and the split exists so `sos.tsx` keeps fast refresh.
...
On `installManageApi`, following `manage.spec.ts`'s harness. **There is no login block, and writing one would be dead code (C1).** `guide.spec.ts` opens with `await installManageApi(page, { staff, replies })`, then `await page.goto(MANAGE)`, then one settle assertion — no login constant, no fill, no submit. `installManageApi` *is* the authentication (`fixtures/manage.ts:14-17`: fulfilling `GET /manage/auth/me` with a 200 `Staff` body is the whole of «signed in»), and the four `LOGIN_SUBMIT` hits in `e2e/` are all `toHaveCount(0)` negatives. Extracting a shared helper is out of scope because **there is nothing to extract**; say so rather than sending a builder hunting for a function that does not exist.
...
6. **One copy-deck rationale is wrong; the string is not (C6).** `copy.md` §3 justifies `checkin.guideHint` avoiding «שליחה» because *"the submit button already owns it"*. The submit is `checkin.submit` = «הצטרפות לתור» (`storefront/src/i18n/he.ts`), not «שליחה». The *rule* is unaffected — the hint still avoids the banned roots and both approved strings stand unchanged. Recorded so a reviewer diffing the deck against `he.ts` does not conclude the string drifted.
