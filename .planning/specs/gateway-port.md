...
| `connect(tenant_id, *, fields, actor_id)` | spend `connect_limiter` → validate shape → build `GatewayCredentials` → `gateway.validate_credentials` → `secret_box.encrypt` → one `tenant_session`: **`pg_advisory_xact_lock(hashtext(tenant_id))`** (see the amendment below), soft-delete the active row, insert the new one at `status='valid'`, write the audit row |
...
> **Amendment (review phase, 2026-07-31) — `connect`'s transaction takes the per-tenant advisory lock.** "One `tenant_session`: soft-delete the active row, insert the new one" is a supersede-then-insert against a partial unique index (`idx_tenant_gateway_credentials_active_unique`) with no serialization. Two simultaneous PUTs — a double-submit, a client retry, two tabs — both read `deleted_at IS NULL` at READ COMMITTED; the loser's UPDATE re-evaluates after the winner commits, matches zero rows, and its INSERT then violates the index. No `IntegrityError` handler is registered anywhere in `main.py`, so that surfaced as an unhandled **500 on an owner-only route**. Fixed with `pg_advisory_xact_lock(hashtext(tenant_id))` — the `create_booking` / `open_deposit` key verbatim — so the loser simply supersedes the winner, which is what a PUT should do anyway; an `IntegrityError` catch mapped to `GatewayUnavailableError` stays as the backstop for a writer that skips the lock, exactly as `open_deposit` maps its own to `PaymentAlreadyHeldError`. **The lock is taken after the ping and the encrypt, not around them** — D4's ordering is unchanged, and a provider hang still never holds a transaction. The test gathers two calls: a sequential loop passes while this is broken, because sequentially the second UPDATE does find the first row.
...
- `settle_from_webhook(tenant_id, *, body, signature) -> Settlement` — `verification_credentials_for` (D20, **not** `credentials_for`) → `gateway.verify_webhook` → then **one guarded statement, not a read-modify-write** (D24):
  ...
  > **Amendment (review phase, 2026-07-31) — the algorithm above never reads `event.paid`, and it must.** Line 187 declares `paid: bool` on `WebhookEvent`; steps 1–3 assign it no consumer. A correctly-signed webhook carrying `paid: false` — the decline/failure notification every real PSP posts to the *same* URL as its successes, Lemon Squeezy included — therefore matched on session id, passed the amount assertion, and settled the hold: `pending → paid`, `paid_at` set, `newly_settled=True`. F19 gates the booking-confirm and F16's confirmation SMS on that flag, so a refused card would have produced a confirmed appointment and a customer SMS with no money taken, and nothing in the audit log to tell it apart from a real one. The field was written by the adapter and read by nobody, so no test could see it either. **The check is a new step between 2 and 3: `if not event.paid`, record and return.**
  >
  > It is deliberately **not** a raise. A decline is a legitimate provider notification, not a forgery — `GatewayWebhookInvalidError` would make the provider retry the decline forever, and it would file a normal business outcome under the same alert as an attempted breach. The branch mirrors the amount-mismatch shape instead: a scrubbed `payments.error`, a `GATEWAY_PAYMENT_DECLINED` audit row, `status` left at `pending`, `newly_settled=False`. Leaving the hold pending is load-bearing twice over — F19's expiry sweeper frees the seat on its own clock, and a retried card can still settle the same hold. Writing `failed` here would pre-empt F19's sweeper policy exactly as the mismatch branch refuses to. The evidence commits on the way out of the transaction it was written in rather than in one of its own, because nothing raises; the db-marked test asserts the audit ROW EXISTS, not merely the return value.
  >
  > **Step 3's paid branch also splits in two.** "The row is now `paid` **with this same txn id**" was written as if it were the only way to reach a paid row, and the shipped code checked only the status. A webhook carrying a *different* transaction id for the same session — a retried card that succeeded twice, a provider duplicate capture — took the identical silent path: `newly_settled=False`, nothing written. That is two real charges for one booking, recorded nowhere but the provider's ledger. The comparison the prose already implies is now made: same txn id → the race, silent as before; a different one → `payments.error` plus a `GATEWAY_DUPLICATE_TRANSACTION` audit row, and still `newly_settled=False`, because the row is right (paid once, by the first txn) and F19 must not double-confirm. Both new actions join the no-actor, failure-path block of `AuditAction`.
