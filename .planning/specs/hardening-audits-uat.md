...
- **Security headers already ship and are already tested on seven surfaces.** `Backend/app/security_headers.py` sets `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, registered LAST in `create_app()` (`main.py:701`) so it is outermost and covers the `TENANT_NOT_FOUND` 404 that `TenantResolutionMiddleware` returns without reaching a handler. **Seven** test files import `SECURITY_HEADERS` and compare the response headers to it with `==` — `test_booking_api.py`, `test_booking_manage_api.py`, `test_checkin_api.py`, `test_notifications_api.py`, `test_payments_webhook_api.py`, `test_spa_serving.py`, `test_storefront_api.py`. *(Amended by the plan, C6: the eighth hit in a naive grep is `app/security_headers.py` itself. The mechanical constraint below is unchanged.)*
...
| **R16** Rate limits per phone **and per IP** — **AMBER at F21, not green** | Per-phone ✓ (5/3600 s send, 10/300 s verify), per-tenant ✓, OTP TTL 300 s ✓ single-use ✓. **Per-IP does not exist on the OTP path at all**, and on login it is conditional on `trust_forwarded_for`, which defaults `False` (`config.py:37`). *(Amended by the plan, C5.)* `_client_ip` already exists (`auth/router.py:29-39`) and **returns `None` on every deployment we currently have**, because `trust_forwarded_for` ships `False` — `config.py:108` names this feature as its resolver in writing. So B5's per-IP key is **code-green at F21 and inert until `TRUST_FORWARDED_FOR=true`**, which is only correct on a deployment terminating exactly one trusted proxy — a host fact. Shipping it and ticking the row green would be a green row on a mechanism that does not run, which is the failure D2 exists to prevent. Three clauses, two owners: per-phone/per-tenant/TTL/single-use **green, F13/F16**; per-IP **code-green at F21, enablement `F62`**; distributed limiter **`F62`**. No `R16a/b/c` split — the checklist already carries AMBER rows with named per-clause owners (F20's R40, R42), and R7 takes the same shape for the same reason. | B5 (per-IP OTP key, shipped inert) + parked clauses (enablement, distributed limiter) |
...
| **R38** Audit log on all owner mutations | **`app/catalog/` has zero `AuditLogRepository` usage** — its **nine mutating endpoints of eleven declared routes** (dress create/update/delete/restore, variants, media presign/confirm/delete/reorder; the other two routes are reads, `GET /dresses` `:139` and `GET /dresses/{dress_id}` `:180`) leave no trail. *(Amended by the plan, C4: nine, not eleven — nine `_audit.record` calls and nine `AuditAction` members. `grep -n audit catalog/service.py` returns nothing, so the "zero usage" half is exactly true.)* `boutique/service.py:143` says outright "`profile` and `toggles` stay UNAUDITED"; appointment types, availability rules/exceptions and terms creation write nothing. `app/queue/` writes nothing (`queue/manage_router.py:29` acknowledges it). | B4 |
...
|---|---|
| **R7 (deployment clause)** — the *live* database role is `boutique_app`, not `postgres` | `walkthrough_coverage_gaps` **G1**: on 2026-08-04 the app connected as `postgres` with `rolsuper = t`, so *"everything the runbook lists as binding under the app role was SILENTLY VOID for the whole run"*. The boot guard exists but is exempt when `app_env == "dev"` (`db/session.py:47`). Proving a deployment's role needs a deployment. **B5 adds what *can* be proved here: that the guard is not exempt for any non-`dev` env value.** *(Amended by the plan, C5: R7 is therefore **AMBER at F21**, the same shape as R16 — the boot-guard clause is green at F21 and pinned by B5; the deployment clause is `F62`'s. Per-clause owners on one row, no `R7a/b` suffix split.)* |
| **R12 (access-restriction clause)** | "Access-restricted" means SSH/console access to the host. No host. |
| **R16 (per-IP *enablement* clause)** | *(Added by the plan, C5.)* F21 ships the per-IP key on OTP send but does **not** flip the flag. `_client_ip` returns `None` unless `TRUST_FORWARDED_FOR=true` (`config.py:37`), which is only correct on a deployment that terminates exactly one trusted proxy. Setting it is a host fact and belongs at stand-up. |
...
**Mechanically, `SECURITY_HEADERS` must stay exactly what it is.** **Seven** test files do `assert {h: resp.headers.get(h) for h in SECURITY_HEADERS} == SECURITY_HEADERS`, which is only meaningful for headers that are unconditional and constant. CSP is settings-derived and HSTS is request-derived, so **neither joins that dict**; they are emitted alongside it by the same middleware and get their own tests. This is the smallest change that adds two headers without touching seven files. *(Amended by the plan, C6 — arithmetic only; the constraint is unchanged and load-bearing.)*
...
`app/catalog/` is the one owner-facing mutation module with **zero** audit rows, and it is the module whose writes are publicly visible. It gets `_audit(...)` on its **nine** mutating paths (of eleven declared routes; two are reads — C4) plus nine new `AuditAction` members — **no migration**, because `audit_log.action` is unconstrained `TEXT`. `platform/service.py::list_tenants` gets a `platform_audit_log` row: it is a cross-tenant read that already takes an operator name and discards it.
...
1. **`/queue` — the public wall board — has no `AXE_ROUTES` row, but it is NOT unscanned.** *(Amended by the plan, C1: the spec's original second clause — "and no bespoke journey" — is **false**, and it was the load-bearing one.)* `Frontend/e2e/storefront.spec.ts` ships **two** gating bespoke journeys, both through the file's shared `axeViolations` (`:571` — `withTags(["wcag2a","wcag2aa"])`, no `.disableRules()`, no `.exclude()`) and both ending in a hard `expect(await axeViolations(page)).toEqual([])`: `:2870` scans the **populated** five-row board (with the 2.2.2 pause taken *before* the scan to make it deterministic against the 5 s repaint), and `:2877` scans the **empty** board as a designed state. So the board's populated and empty states are **closed**. What `public-queue-board.md`'s **A29** ("zero axe violations on every materially different state") still lacks must be **derived from `QueueBoardPage.tsx`'s own render branches at build time**, not assumed to be "all of it" — most likely the truncated/partial state and the outage state, both of which render different chrome. **If that enumeration finds nothing left, A29 is ticked with the two shipped journeys as its evidence and this clause costs zero lines. That is a legitimate outcome and is recorded as one** — a third scan of the same DOM is coverage theatre. It is public, unauthenticated and wall-mounted.
2. **Eleven of fifteen console sections have no axe scan**: `dashboard, profile, hours, types, terms, catalog, bookings, customers, staff, gateway, privacy`. *(Amended by the plan, C3: the **list is exactly right**; the original count — 9 of 16 — was wrong twice over.)* `guide.ts:14-33` declares `SectionKey` with **fifteen** members, and `GUIDE_STEPS` `:53-69` is the fifteen-key `satisfies Record<SectionKey, …>` that makes a sixteenth a type error. **Four** are already scanned: `floor` (`manage.spec.ts:601`), `board` (`:643`), `atelier` (`atelier-capacity.spec.ts:291, :370, :491`) and — *incidentally* — `dashboard`, via `guide.spec.ts:625`, since `gotoConsole` lands on the first `NAV` row and a `dialog:modal` leaves the rest inert but visible to axe (`manage.spec.ts:419`). **`dashboard` stays on the build list anyway**: a scan taken with a modal open is a scan of a different DOM, and this row's whole complaint is that silence reads as coverage. Its own scan is one `AXE_SECTIONS` row. Two matter most — `staff`, which is where F61's nameless-button defect lived, and `privacy`, which is the §13 subject-export/erase surface.
...
- **B2** — per-header tests on a real response for both apps' shells, the JSON API, and the `TENANT_NOT_FOUND` 404; a settings-derived test proving the media origin appears in `img-src`/`connect-src` when a bucket is configured and **is absent when it is not**; an HSTS test proving presence over https and absence over http. **And the one that matters most: a Playwright test loading the real built bundle with the real policy applied and asserting zero CSP violations** — a header-string assertion cannot see `assetsInlineLimit` turning an icon into a `data:` URI.

  ⚠ *(Amended by the plan, C2 — the acceptance criterion stands, the **instrument** changes, because the one this bullet originally implied cannot work.)* **The e2e harness never touches the middleware.** `Frontend/e2e/playwright.config.ts:14-29` declares two `webServer` entries and both are `vite preview` (`--filter storefront preview --port 4173`, `--filter manage preview --port 4174`); `Makefile:51-52` confirms it. **FastAPI is not in the request path in e2e**, so the CSP `SecurityHeadersMiddleware` emits is invisible to every Playwright test that exists, and a spec file added naively would assert the *absence* of a header and pass vacuously — the exact `known_vacuous` failure mode this repo has been bitten by four times. A third `webServer` running uvicorn was rejected (it needs a database, a migrated schema, a seeded tenant and a resolvable `Host`, since `TenantResolutionMiddleware` 404s an unknown host — a whole new harness for one header), and `preview.headers` in the two `vite.config.ts` files was rejected (the policy becomes a copy in two frontend configs, drifting the moment `media_endpoint_url` changes, and proving nothing about the middleware).

  **The instrument is: Playwright `page.route` interception injecting a policy read from a committed fixture (`frontend/e2e/fixtures/csp.txt`), with a backend test pinning that fixture byte-for-byte to what the middleware actually emits.** The browser applies a real policy to the real bundle, and drift between the two halves is a **red backend test**, not a silent divergence. The parity direction has precedent here: `test_frontend_constant_parity.py` and `test_frontend_imports_are_tracked.py` both read the frontend tree from a backend test. The browser test additionally carries an anti-vacuity leg — it asserts the app's own root heading rendered, because a CSP that blocks the module script leaves a blank page, and a blank page also has zero *further* violations. **Plan Tasks 5 and 6 spell this out.**
