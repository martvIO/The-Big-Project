...
- [ ] **`role="seamstress"` renders no «שעות» on any row and no «הגדרות». Mutation: drop the role guard → a 403 reaches `runMutation`, `poll.fail` classifies it terminal and the whole section is replaced by «אין הרשאה» → red** → `SeamstressPanel.test.tsx` + `AtelierSection.test.tsx`
