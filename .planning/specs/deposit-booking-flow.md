...
**D3a — `provider_session_id` is ONE identifier space, and it is the reference.** Found during F18's build and fixed there, recorded here because F19 is where it would have cost money. `open_deposit` passes `reference=str(booking_id)` and stores whatever the adapter returns as `provider_session_id`; `settle_from_webhook` looks the row up by the `provider_session_id` the adapter reads back off the webhook. F18's first cut returned the provider's *checkout id* on the create half and the *reference* on the webhook half — two different identifier spaces, so a verified, genuinely-paid webhook would have matched no row: charge taken, payment left `pending`, seat freed by the sweeper, bride confirmed nowhere.

The port implies one space and every adapter must honour it. The reference is the only value present on both halves (a Lemon Squeezy order carries no checkout id, and `checkout_data.custom` is fixed before the checkout id exists). **F19 must not paper over this with a lookup fallback** — a test asserting the two halves agree lives in the adapter suite, and any future adapter needs the same one.
