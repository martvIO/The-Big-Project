...
|---|------|--------|--------|-------|
| 1 | AWS account + il-central-1 opt-in | not-started | E2 #8 (S3 upload), E4 skeleton, production | Create account, enable the il-central-1 opt-in region, billing alarm. Fastest item — do first. **Re-flagged 2026-07-24 at F8 kickoff.** F8 ships its upload code behind a storage port, CI-verified against MinIO, so **F8 is not blocked** — the bucket reduces to configuration. **The binding gate is F10, not F8**: every approved storefront screen is photography-led, so the account must be `approved` and the staging presign → upload → confirm → signed-GET smoke green **before F10's spec is written**, or E2 closes on a monogram-only storefront. Pilot consequence: do not have the boutique enter catalog data before the bucket is live, or they revisit ~60 dresses to add photos. |
