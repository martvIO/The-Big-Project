...
**PRE-1 was worked around in `prototype-test.html`** (override block, item 8) — otherwise roughly one B4 tap in seven would land on a dead accessibility button and read as "the booking button is broken", destroying the finding B4 exists to produce. **The design fix is now in the tokens** (`--space-a11y-clearance`, 2026-07-25); the F9 build wires the `A11yMenu` button to it and the build QA asserts zero overlap.
