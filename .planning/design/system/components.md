...
| `DressCard` | 3:4 image (object-cover, cream matting on letterbox), name, price OR `PriceOnRequest` treatment, status badge only when meaningful | image-missing (monogram placeholder), reserved |
...
| `PriceTag` | honors per-dress visibility: value in ink, "מחיר בתיאום" in muted italic — same layout slot, no jump | — |
...
| `ContactPanel` | tap-to-call, WhatsApp, address→maps, Instagram | — |
| `A11yStatementLink` | footer link to `/accessibility` (IS 5568 obligation) | — |
