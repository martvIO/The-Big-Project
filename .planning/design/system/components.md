...
| `ButtonLink` | **F14.** Renders `<a href>` with `Button`'s exact class recipe (shares its private `base`/`variants`/`sizes`/`focusRing`), for controls whose whole job is to change the URL. Deliberately has **no `loading`, no `disabled`, no `type`, no `ref`** — a link can be none of those, and a "disabled anchor" is the commonest way a design system ships an unreachable control. **The href must be absolute**: the router's delegation pushes the raw `getAttribute("href")`, so a relative one breaks | — |
| `Input` / `TextArea` | label REQUIRED (never placeholder-as-label), help text, char counter (terms — the count is wrapped in `<bdi dir="ltr">`, R25), `dir` override for phone/URL fields (LTR content in RTL form). **All three of `Input`/`TextArea`/`Select` declare `ref`** (React 19 ref-as-prop; it is not part of `*HTMLAttributes`, so it must be declared explicitly) — without it there is no way to programmatically focus a field, which makes focus-to-first-invalid and OTP focus retention unbuildable | error (message tied via `aria-describedby`), disabled |
...
| `Checkbox` | **F14.** Real `<input type="checkbox">` with **no role override** — for one-shot legal consent (terms acceptance), which must announce checked/unchecked, not on/off. That is precisely why it is not `Toggle`, which hardcodes `role="switch"` with no opt-out. Visible label required; the `<label>` wraps box + text at 44px min block size around a 24×24 box; error wiring identical to `Input`'s | error (`aria-describedby` + `role="alert"`), disabled |
...
| `BookingCTA` | persistent bottom bar @mobile, inline @desktop. **Since F14 (E3 #14) its action is a `ButtonLink` into `/book/slot`** — on a dress page `/book/slot/{dressId}`, a path segment because the navigation store snapshots `pathname` only and cannot see a query string. The old contact-panel modal is deleted (**D1**), and with it the `boutique` prop: a control that only navigates needs no boutique data, so the CTA now renders **even when the boutique fetch fails** (**D12**). It is deliberately absent from `/book/*` — a "book" CTA inside the booking flow is the inverse mistake | — |
