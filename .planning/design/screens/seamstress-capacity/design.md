...
- **P-6 — RESOLVED: a seamstress sees every row, every bar and every sentence, and no control at all.** No disabled button, no lock glyph, no «אין לך הרשאה» line — F41's §2.3 reasoning, plus one that is specific to this panel: she is admitted to the board by the router and refused by **both** write routes, so an ungated control is one whose only possible outcome is a **403** — a button the console offers her and the server is certain to refuse. *(Corrected in review round 1: this bullet previously argued that the 403 would reach `poll.fail` and replace **her entire board** with «אין הרשאה». It would not — both panel writes pass `terminalOnFailure = false`, so a per-route refusal means "not this control", never "not this board". That opt-out is the real protection and is pinned separately; do not remove it to make the old wording true.)* The absence is cosmetics; the control is the server's per-route gate.
