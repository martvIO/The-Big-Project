...
8. **Status is carried by WORDS.** A `Badge`'s colour is reinforcement and never the signal — F51's shipped rule (*"The WORD carries the role; the colour never does"*) and `FloorPanel.tsx:554`. ⚠ **On this feature that is not a formality**: the surface is a full-screen red field, and `tokens.md` law 2 is satisfied only because every state — «פתוחה», «מטופלת», «ללא מענה», «אין תזוזה מאז שאושרה» — is a Hebrew word. **Remove every colour from this feature and nothing is lost but emphasis. No emoji, no dots, no glyphs**: an emoji is announced by a screen reader with a name this product did not choose and cannot translate, and the console ships **no icon vocabulary at all**.
...
⚠ **The array now folds NINE constants and F37 makes it ten** — `HE_F33` joined after F33 merged, so the spec's `:61-71` line reference is two lines stale. The mechanism is unchanged; **check the line numbers, not the count, when adding the spread.**

Without the fold, all 48 hand-transcribed strings ship unchecked for the exclamation mark, for the `/נשלח|תישלח|בדרך/` send-ban — **the ban the entire «אני מגיעה» wording decision rests on** — and for a missing `ar` key.
...
| — | **Line 2, WHERE.** ⚠ **NOT A KEY.** The room label renders as a bare `<bdi>` in its own element with no prefix and no interpolated sentence (`design.md` **P-5**, rule 6). ⚠ **CORRECTS D17**, whose `sos.room` row already carries the reasoning but is listed as a key | — | — | ⚠ **CORRECTS D17** |
...
| Key | What it must say | Approved Hebrew (`he`) | `ar` (untranslated) | Status |
|---|---|---|---|---|
| `sos.accept` | **The single most important button in this feature.** `Button primary lg fullWidthMobile`, 48px, first in DOM so the default outcome of a keypress inside the overlay is *accepting the emergency*. ⚠ **NOT «אני בדרך»**, which is the natural Hebrew and trips the global `/נשלח\|תישלח\|בדרך/` ban (rule 2). «אני מגיעה» is first-person, present-continuous and commits her — «בסדר» or «קיבלתי» would acknowledge a *message*; this acknowledges a *person*, and the raiser is told the same word back | אני מגיעה | אני מגיעה | DRAFTED |
...
## 8. Reused — SHIPPED, never re-declared, never counted into the 48
...
## 9. The scan — every rule, mechanically, over all 48 values
...
| **11 — `ar[key] === he[key]`** | all 48 rows | ✓ by construction — the two columns of this file are byte-identical, which is what the spec's assertion checks and what makes this deck **one file to one file** |

---

## 10. Landing checklist

1. **Transcribe all 48 rows into `he.ts` and `ar.ts`, from this file and not from spec D17's table.** Five rows differ (§0, §2, §3, §5, §6) and two are additions D17 does not have (§7.1).
...
3. **Add the `sos.*`-scoped assertions**: every key resolves in `he` and in `ar`; `ar[key] === he[key]` for every key (**not** "non-empty"); no value matches `/נשלח|תישלח|בדרך/`; no value contains a literal digit.
