...
| `--color-ink` `#2B2118` | **every shipped `Button secondary` and `ghost` label**, every `Badge neutral` | **2.22:1** | ✗ — fails AA text |
| `--color-focus` `#7F612B` | **the one focus ring in the product**, `focusRing`, drawn at `outline-offset-2` i.e. **on whatever is behind the control** | **1.30:1** | ✗ — **invisible**, and it is the *only* focus indicator this console has |
| `--color-danger` `#A03232` | `Button danger`'s own fill | **1.00:1** | ✗ — the emergency's most natural button disappears |

Every shipped `packages/ui` control assumes a light surface. `secondary` is `border border-ink bg-transparent text-ink`; `ghost` is `bg-transparent text-ink`; `danger` is `bg-danger text-surface-raised`; every `Badge` variant is a border plus coloured text. **Put the ack control directly on `bg-danger` and the product's whole component vocabulary either fails contrast or vanishes** — and `focusRing` at 1.30:1 is a WCAG 2.4.7 failure on an emergency control, on a surface where IS 5568 is legally binding, **that axe cannot report**: its contrast rule computes an element against its own background and does not model an `outline` drawn on a parent.
...
*Two ledger rows are added at this gate and both are negative:* **ink on danger 2.22:1 ✗** and **focus on danger 1.30:1 ✗**. They are recorded so the next feature that wants a coloured field meets the measurement rather than the absence.
...
**«אני מגיעה» — `Button variant="primary" size="lg" fullWidthMobile`.** Gold fill, ink label, **6.41:1** (tokens ledger), `min-h-12` = 48px, full-bleed across the card's 295px at 375 and auto-width from `sm` up. First in DOM, therefore first reached by Tab once focus is in the overlay — **so the default outcome of a keypress inside the overlay is accepting the emergency, not hiding it.**
...
**The five things that make an accidental accept improbable, and none of them is a second tap:**
...
1. **A word.** «ללא מענה» in `text-danger font-semibold`, in a **sibling node outside** the `role="alert"` region (spec D15, AC16). `--color-danger` on white is ≈7.4:1 ✓. Never a border colour, never a background swap, never a second `Badge` — F51's shipped rule (*"The WORD carries the role; the colour never does"*) and `FloorPanel.tsx:554`.
...
+--------------------------------------------------+   fixed inset-inline-0 bottom-0 z-40
```
...
| **S-one** | One rising alert | poll tick | §2.2 — one card, announced once | `role="alert"`; **MOVE A** iff `activeElement === body` |
...
| **S-gone** | Somebody else accepted while it was up | next tick, `status !== "open"` | the card leaves. **No error, no message** — she did not lose a race, somebody answered | **MOVE C**, or **MOVE B** |
| **S-mine** | This caller accepted | 200 | the card leaves; `SosCentre` shows «מטופלת» | **MOVE B/C** + a `role="status"` **toast** (§9.5) |
...
| Raised by me, unanswered | my own row, **no accept control**, cancel + resolve |
...
| Escalation / stall clause | `<p className="text-base font-semibold text-danger">` — a **sibling** of the alert region. `--color-danger` on `--color-surface-raised` ≈ 7.4:1 ✓ |
...
| Bottom container | `<div className="fixed inset-inline-0 bottom-0 z-40 flex flex-col items-end gap-2 p-4">` — one container, so the strip and the affordance can never collide |
| Channel strip | `<div className="w-full rounded-md border border-danger bg-surface-raised p-3">` + `<p role="alert" className="text-sm text-ink">` + `Button variant="secondary" size="md"`. F57's terminal-panel shape (`FloorPanel.tsx:446-456`) |
| Re-open affordance | `Button variant="danger" size="md"` — white on danger **7.00:1 ✓**, focus ring on the console's cream **5.57:1 ✓**, ≥44×44 |
| SOS-centre heading | `<h3 ref={sosHeading} tabIndex={-1} className="text-base font-semibold text-ink">` — matches F36's rooms `h3` exactly; `tabIndex={-1}` adds **no** tab stop and is MOVE G's fallback-of-last-resort |
...
| SOS-centre list | `<Card>` → `<ul className="divide-y divide-border">` → `<li data-alert-id={id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-start">` — `FloorPanel.tsx:521` / `RoomsPanel`'s exact shape |
...
**Contrast, computed at this gate — not eyeballed.** ink on surface-raised ≈16.1 · ink on surface (paper) 13.89 · ink-muted on paper 5.61, on white 6.36 · danger text on white ≈7.4, on paper 6.18 · warning-text on white 5.90, on paper 5.20 · white on danger **7.00** · gold/ink 6.41 · focus ring on cream 5.57, on paper 5.08, on white 5.76. **Two negative rows enter the ledger at this gate** (§2.1): **ink on danger 2.22 ✗** and **focus on danger 1.30 ✗**. **No gold-strong anywhere; this feature adds no new colour and no new token.**
...
- **The announced content is three block children of the region — who, where, note** — read as one atomic utterance: «דנה כהן קוראת לעזרה … חדר 2 … צריך סיכות». They are the **same nodes** the sighted user reads, so nothing is duplicated for a screen reader and there is no visually-hidden copy to keep in sync.
...
### 9.2 Focus — seven moves, each with a named, non-vacuous mutation
...
|---|---|---|---|---|
| **A** | The first rising alert appears | **iff `document.activeElement === document.body`** | that card's «אני מגיעה» | delete the `=== document.body` guard, then assert focus does not leave a text input |
| **B** | The overlay unmounts while holding focus | `activeElement` was inside it | `document.getElementById("console-main")` — the `<main tabIndex={-1}>` `ConsoleShell` already renders and the skip link already targets (`ConsoleShell.tsx:43,84`). **Never `<body>`** | delete the effect |
| **C** | A card leaves with siblings remaining | `activeElement` was inside that card | the **next remaining** card's accept control; falls through to B when none remains | delete the departing-card check |
...
| **G** | The raise dialog closes | `activeElement === document.body` | **`FloorPanel`'s own `sosTriggerRef`**, `trigger.isConnected ? trigger.focus() : sosHeading.current?.focus()`. **Never `<body>`** | delete the `isConnected` branch — focus goes nowhere when the tile was released underneath |
...
- **Visible focus ring on every interactive element** — `focusRing`, applied unconditionally by `Button.tsx:62`, `Input.tsx:45`, `Select.tsx:31`. Nothing sets `outline: none`. ⚠ **And §2.1 is what makes this true rather than nominal**: on `bg-danger` the shipped ring is 1.30:1 and axe would not say so.
