...
| `dashboard.firstRunNote` | Day one. Shown only while the whole history is zero, as one muted line under the heading — **never an `EmptyState`**, which would hide the forward panel (spec D10, Risk 1). It explains that the screen fills itself, states the fact that the numbers are zero, and promises nothing about when | המסך הזה מתמלא מעצמו ככל שנקבעים תורים. עד אז המספרים כאן הם אפס. | המסך הזה מתמלא מעצמו ככל שנקבעים תורים. עד אז המספרים כאן הם אפס. | DRAFTED |
