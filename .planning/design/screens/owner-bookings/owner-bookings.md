...
| **DA-fail** | Detail · action failure | 409 `BOOKING_TRANSITION_INVALID` / `SLOT_UNAVAILABLE` / `CUSTOMER_ALREADY_BOOKED`, 429 `TOO_MANY_ATTEMPTS`, 400 `VALIDATION_ERROR` | `<p role="alert" className="text-sm text-danger">` in the action group — **fix-this** register. The row is **re-rendered from the response the mutation returned** where one exists; a 409 never leaves the screen showing a state the server refused | alert, and the alert is `tabIndex={-1}` and **focused**. Not the control: every action Button carries `disabled={busy}`, which blurs it on tap, and the two confirm Modals unmount in the very commit that disables their trigger — so `.focus()` on the trigger is a no-op and focus sits on `<body>` (WCAG 2.4.3). The phone `400` renders in the `Input`'s own error slot instead, and focuses the field |
...
| **RD-readfail** | Reschedule · slot-read failure | `/manage/slots` non-2xx / network | `<p role="alert" className="text-sm text-ink-muted">` — **outage** register — **plus a «ניסיון נוסף» secondary Button**. Unlike **L-fail**, this branch cannot decline a retry control: the alert *replaces* `SlotPicker`, which owns the DateField that would otherwise refetch, so without the button the dialog holds one disabled control and «חזרה» | alert |
...
**State precedence when an action races reality.** Every mutation answers the same `OwnerBookingDetail` (D7), so a success always re-renders the whole detail from the response and never from what the client hoped. A 409 renders **DA-fail** and leaves the previously-rendered facts in place; the owner's recovery is the back control — the console never guesses a new state from an error. The day list is refetched when a **reschedule** returns a different `starts_at`, because list membership, the server `total` and the server `(starts_at, seat_index)` order are all derived from it and no in-place patch can keep those three honest; every other mutation patches the row from the response, since the two views then render one object.
...
| Value | Wrapper | Why |
|---|---|---|
| time (`10:00`), date (`4.8.2026`), phone (`+972501234567`), terms version (`3`), seat index (`2`), the day count | **`<bdi dir="ltr">`** | numeric/Latin runs: without the explicit direction, a mixed run reorders around the surrounding Hebrew and a phone number renders with its digits in the wrong order |
| customer name, dress name, **`notes`**, **dress size** | **bare `<bdi>`** | free text authored by a person. `dir="ltr"` on Hebrew is **itself a bidi defect** (`BookPage.tsx:1019-1022`) — it forces Hebrew to render left-to-right. Bare `<bdi>` isolates the run and lets the UA infer direction per content, which is the only correct answer for a field that may hold Hebrew, Arabic, Latin or a mix |
...
**Dress size is in the second row, not the first, and this table used to say otherwise.** `bookings.dress_size` snapshots `dress_variants.size_label` — unbounded owner-typed `TEXT` with no numeric constraint (`catalog/validation.py` collapses whitespace and nothing else), so «מידה 36» or «S ארוך» is an ordinary label and `dir="ltr"` puts its digits on the wrong side of its Hebrew. A bare `<bdi>` still renders a plain `36` LTR, because `dir=auto` has no strong character to disagree with, so the second row is strictly safer for this field. The storefront already reads it as owner-authored free text (`ManageBookingPage.tsx`, `BookPage.tsx`, both bare, the latter locked by test R19).
