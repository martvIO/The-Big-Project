...
| | |
|---|---|
| Rows in this deck | 1 (`document.book`) + 45 (`booking.*` from the spec's inventory) + 6 (`errors.*`) + 9 (additions, §4 — seven proposed + two added at sign-off) + 3 (build-time additions: `typeRequired`, `timeRequired`, `loading` — approved 2026-07-29) + 2 (rev 5 R19 sentence tails: `refundWindowSuffix`, `forfeitSuffix`) + 1 (rev 5 build-time: `invalidCharacters`) = **67**, plus one pre-existing key documented on a new surface (`errors.validation`) |
...
| Gate status | **APPROVED — 67 of 67 rows, 2026-07-29** |
