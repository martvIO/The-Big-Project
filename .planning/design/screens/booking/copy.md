...
**Date**: 2026-07-29 · **Status**: **rev 3 — SIGNED OFF 2026-07-29; every row APPROVED** · **Owner of the Hebrew: the user** · **Consumes**: `.planning/specs/storefront-booking-ui.md` §Design i18n inventory (verbatim key list) · **Lands in**: `Frontend/apps/storefront/src/i18n/he.ts`
...
| `booking.typeDuration` | Under each appointment type's name. **`{{minutes}}`** = `duration_minutes` from `GET /storefront/appointment-types` | How long that appointment takes. Reads as a fact, not a constraint. | ‏{{minutes}} דקות | APPROVED |
...
| `booking.forDress` | **ADDITION.** A non-interactive `Badge` above the type picker on the item-based path only. **`{{dress}}`** = the dress name | Reassures her the flow remembered which dress she came from. Reads as a label, not as an editable choice — there is no unbind control. | עבור {{dress}} | APPROVED |
...
| `booking.refundWindow` | Above the policy text, as a plain-language summary. **`{{hours}}`** = `refundable_until_hours_before` from `GET /storefront/terms` | The free-cancellation window, in plain words, so she does not have to infer it from a paragraph. | ביטול עד {{hours}} שעות לפני המועד — ללא חיוב. | APPROVED |
| `booking.forfeit` | Directly under `refundWindow`. **`{{percent}}`** = `forfeit_percent` | What is charged for a late cancellation or a no-show. | ביטול מאוחר יותר, או אי-הגעה — חיוב של {{percent}}% מהמקדמה. | APPROVED |
...
| `booking.confirmTitleNamed` | **ADDITION (gate sign-off 2026-07-29, closes §7 Q4 as "yes").** The confirmation screen's `h1` when boutique data is in memory. **`{{name}}`** = the boutique's name. An i18next string cannot be conditional, so the named/nameless split needs two keys; shipped precedent for `{{name}}`: `statement.coordinatorNoChannel` | It is done, at this boutique — so a screenshot stays self-explanatory weeks later. The generic "חנות הכלות" fallback must never appear on her only record; that is what `confirmTitle` is for. | התור נקבע ב{{name}} | APPROVED |
...
| `booking.confirmDress` | **ADDITION — not in the spec's inventory.** The dress line on the item-based path only. **`{{dress}}`** = `dress_name` · **`{{size}}`** = `dress_size` | Which dress and which size she is coming to try. Omitted entirely on the generic path. | {{dress}} · מידה {{size}} | APPROVED |
...
| | |
|---|---|
| Rows in this deck | 1 (`document.book`) + 45 (`booking.*` from the spec's inventory) + 6 (`errors.*`) + 9 (additions, §4 — seven proposed + two added at sign-off) + 3 (build-time additions: `typeRequired`, `timeRequired`, `loading` — approved 2026-07-29) = **64** |
...
| Gate status | **APPROVED — 64 of 64 rows, 2026-07-29** |
