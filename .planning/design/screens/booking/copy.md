...
**Rev 5 also carries three copy decisions from review round 2, all user-approved**: `errors.otpSendBudget` is **reworded to attribute nothing** (it accused a bride who had asked for one code, because `OtpService` raises its `429` only on the per-*tenant* ceiling — while the bride who really spent her personal five was told "code sent" four more times, falsely; the client now counts her own five and takes the same exit); a new `errors.bookingBudget` gives the booking `429` its own face instead of R13's "try again" over an hour-long window; and a new `booking.invalidCharacters` covers the control-character gate the client had never mirrored. `errors.smsUnavailable` keeps its exact words and gains a second *shape* — see its row.
...
| `booking.invalidCharacters` | **ADDITION (build-time, review round 2).** Field error on **either** free-text field when it carries a control character. Mirrors `_CONTROL_CHARS` / `_CONTROL_CHARS_EXCEPT_WS` in `Backend/app/booking/validation.py` — the name bars all of C0 plus DEL (a line break in a value F16 will template into an SMS is header-injection material), the notes keep `\t` `\n` `\r`. Without it the client blessed a value the API refused with a `400` **after** she had spent an SMS, and every retry failed identically | The text carries something we cannot save, and here is the thing she can actually act on. **The character is invisible**, so the string names the *paste* — which is what she can see and redo — rather than the byte, which she cannot | יש כאן תו שאי אפשר לשמור. אם הדבקת את הטקסט, אפשר להקליד אותו מחדש. | APPROVED (build-time, review round 2) |
...
| `errors.smsUnavailable` | `503 SMS_NOT_CONFIGURED` **or** `503 SMS_UNAVAILABLE` — one string for both codes (rule 10). **Rev 5: one string, two shapes.** `SMS_NOT_CONFIGURED` is known at boot and permanent, and keeps the whole-form dead end. `SMS_UNAVAILABLE` is one failed provider send and transient by nature, so it becomes a step-level alert with the contact panel and the form left standing — the dead end is never cleared while the page stays mounted, so routing a blip there ended the session for good. **Rule 10 is untouched: the bride still cannot tell which code fired, because the words are identical** | Phone verification is down, so this booking cannot be completed here. **Say it honestly and hand her the phone.** She must not be left retrying. | אימות הטלפון אינו זמין כרגע, ולכן אי אפשר להשלים כאן את קביעת התור. נשמח שתתקשרי אלינו ונקבע יחד מועד. | APPROVED (wording unchanged) |
| `errors.otpSendBudget` | **ADDITION (R21), REWORDED rev 5 (user-approved, review round 2).** A spent `/otp/send` budget, in **both** of its faces: the `429` the per-TENANT ceiling raises, and — new — the client's own count of five sends in the session, past which the server answers a real send and a spent personal budget with the same silent `204`. Distinct from `errors.tooManyAttempts`, which covers a spent *verify* or read budget | **Attributes nothing to anyone.** The original said "*you* asked for several codes in a short time", and `OtpService.send` raises `OtpThrottledError` **only** on the per-tenant ceiling — so that sentence accused a bride who asked for one code on a busy boutique, while the bride who really spent her five saw "code sent" four more times, falsely. One sentence that is true of both faces, offering the phone rather than a retry. | אימות הטלפון עמוס כרגע. אפשר לנסות שוב בעוד זמן מה, ואפשר פשוט להתקשר אלינו ונקבע יחד מועד. | APPROVED (rev 5 — user-approved reword) |
| `errors.bookingBudget` | **ADDITION (rev 5, user-approved, review round 2).** A `429 TOO_MANY_ATTEMPTS` on `POST /storefront/bookings` — the create budget. It previously fell into R13's `errors.unknown` ("נסי שוב") for a window that lasts about an hour. Rendered in the step-level block with the contact panel under it, the same shape `errors.otpSendBudget` takes | Several booking attempts in a short time; the wait is roughly an hour, so **offer the phone rather than a button that cannot work for that long.** | ניסית לקבוע כמה תורים בזמן קצר. אפשר לנסות שוב עוד כשעה, ואפשר פשוט להתקשר אלינו ונקבע יחד מועד. | APPROVED (rev 5 — user-approved) |
...
| `errors.validation` | **Pre-existing key, new surface (review round 2).** A `400 VALIDATION_ERROR` on `POST /storefront/bookings`. It now routes her back to `/book/details` — the step that owns the two free-text fields — and renders in the same slot the terms step's return notice takes. Previously it fell into R13's catch-all, where "try again" was a permanent dead end: the body is rejected deterministically, so every press re-sent the same bytes and spent another slot of a ten-per-hour create budget | The request could not be accepted; the fields that could have caused it are on the screen she lands on. **Wording unchanged** — the key already existed and already said this | הבקשה לא תקינה. | APPROVED (unchanged; new surface) |
...
The spec's §Design i18n inventory is the fixed key list. This deck proposed **seven** keys beyond it, and the sign-off session added **two** more — all **nine approved 2026-07-29**. Rev 5 adds **four**: two are R19 sentence tails carrying words already approved on their own row, and two are new copy (one user-approved, one build-time). None replaces a spec key; each covers something the inventory cannot express.
...
| `booking.refundWindowSuffix` | **Rev 5 / R19 — a shape, not new copy.** `refundWindow`'s value sits mid-sentence, so the split R19 requires cannot be a lead alone: the words after the isolated number need a key to live in. Holds the second half of the already-approved sentence, verbatim. | **APPROVED — rev 5** |
| `booking.forfeitSuffix` | **Rev 5 / R19 — a shape, not new copy.** Same, for `forfeit`. | **APPROVED — rev 5** |
| `booking.invalidCharacters` | **Rev 5, review round 2 — not optional.** `Backend/app/booking/validation.py` rejects control characters in `name` and in `notes`, and the client mirrored neither, so a paste out of a word processor passed the form, **spent an SMS**, and drew a `400` that no retry could clear. Mirroring the gate means the field needs a message, and the deck had none. | **APPROVED — rev 5** |
| `errors.bookingBudget` | **Rev 5, review round 2, user-approved.** A `429` on `POST /bookings` is a distinct face from a `429` on a read: the window is about an hour, and R13's `errors.unknown` ("נסי שוב") put her on a button that could not work for that long. Same reasoning that gave `/otp/send` its own key under R21. | **APPROVED — rev 5** |
...
| | |
|---|---|
| Rows in this deck | 1 (`document.book`) + 45 (`booking.*` from the spec's inventory) + 6 (`errors.*`) + 9 (additions, §4 — seven proposed + two added at sign-off) + 3 (build-time additions: `typeRequired`, `timeRequired`, `loading` — approved 2026-07-29) + 2 (rev 5 R19 sentence tails: `refundWindowSuffix`, `forfeitSuffix`) + 2 (rev 5 additions: `booking.invalidCharacters`, `errors.bookingBudget`) = **68**, plus one pre-existing key documented on a new surface (`errors.validation`) |
...
| Gate status | **APPROVED — 68 of 68 rows, 2026-07-29** |
