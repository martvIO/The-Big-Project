...
> **RESOLVED in the F14 build (2026-07-29, commit `f73a3c6`) — the user approved folding it into this feature** rather than leaving a statutory-link obstruction unowned on every storefront route. The remedy is the one predicted above, and the diagnosis held exactly: page-level padding can never clear the trigger, because `<footer>` is a sibling of `<main>` and every page reservation (including the shell's own `--cta-bar-height`) sits inside the element that is *not* being covered. **The footer now pays the reservation**: `padding-block-end: var(--space-a11y-footprint)`, where the new token is `calc(44px + var(--space-4) + var(--space-2))` — the trigger's own box, plus its inset, plus one step of air — derived exactly as `--cta-bar-height` is, never a literal. The lifted case is covered too: where `--space-a11y-clearance` raises the trigger to 92px the shell already reserves 80px, leaving a 56px residual under the 68px reservation. Neither element is hidden, moved off-screen or un-fixed. Pinned by a `StorefrontLayout` unit block across five routes (jsdom has no layout, so it asserts the reservation and that no `pb-N` literal creeps back) and by `qa-checklist.md` PRE-2's geometry loop in e2e, widened to measure **every** footer link rather than only the CTA.
