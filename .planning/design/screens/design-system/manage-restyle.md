...
| `HoursSection` | weekly editor as a Sun-first grid; windows as rows with TimeFields; capacity as a small numeric field with "מקבילים" label; exceptions list — date + closed/special chips, `Button variant="danger"` remove buttons; validation errors inline per row (house error message under the row) |
| `TypesSection` | type cards in a list (name display-serif sm, duration/audience/deposit as muted meta line); archive = `Button variant="danger"` trigger with confirm Modal; agorot fields render ₪ presentation with `dir="ltr"` digits |
...
| `CatalogSection` (F8) | dress list: search `Input` (`dir` neutral), sort-order control, "הוספת שמלה" primary Button, dress rows as `Card`s with name (display-serif sm), `Price`, status `Badge` (`muted` "במלאי (N)" / `warning` "אזל מהמלאי" / `muted` "בארכיון"); the archive **trigger lives in `DressEditor`, not here** — `CatalogSection` ships an "ארכיון" filter `Toggle` only. Restyle only — F8 API/behavior/tests frozen |
...
### Destructive actions — one pattern, corrected 2026-07-30

**`Button variant="danger"` trigger → `Modal` whose footer is `ghost` dismiss + `danger` confirm.** That is what all three shipped destructive acts do: `TypesSection.tsx:276` (archive), `DressEditor.tsx:401` (archive), `HoursSection.tsx:305` (remove exception). Trigger and confirm are never co-visible — the Modal covers the panel — so two `danger` buttons in one component is correct, not a hierarchy failure.

Until 2026-07-30 the three rows above described this as "ghost-danger" / "danger-ghost", and the F15 deck inherited the error. **There is no such variant** — `Button.tsx:4` defines `primary | secondary | ghost | danger` — and the combination could not have worked if written: `cn()` (`packages/ui/src/lib/styles.ts:4`) is a plain join with no class-merge, so a consumer `text-danger` and ghost's baked-in `text-ink` are same-specificity rules resolved by stylesheet order, and the built CSS emits `.text-danger` **before** `.text-ink` — ink wins, and the button renders as an ordinary ghost. The same trap voids a consumer `p-0` against `Card`'s own `p-6`. **General rule: never override a `packages/ui` component's own utility from the call site — add the variant or the prop to the component.** See `owner-bookings.md` F-6.
