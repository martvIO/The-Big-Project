...
- [ ] i18next wired with Hebrew as default locale. **`packages/ui` components carry zero Hebrew** (all copy via props — the binding rule). App-side: net-new F9 chrome (shell nav/titles, LoginForm, ProfileSection) is i18n-keyed; F7/F8 section copy (Catalog/Dress/Variant/Media/Hours/Types/Terms) stays **inline** for now — `manage-restyle.md` scopes those as "restyle only, visual layer", and their frozen tests assert the exact strings. Full section i18n extraction is a tracked follow-up, not an F9 blocker
...
- [ ] Detail 55/45 @768 with thumbs · 60/40 @1440 with sticky facts column · console nav stacks full-width @375 → horizontal tab row @≥768 (contract (b) panel-swap, not a disclosure accordion)
...
- [x] **PRE-2** — on `/about` and the console (no fixed bar) the scrollable page reserves bottom padding ≥ the button's footprint, so it never rests on a Waze link or a form input. **Closed in F14 (`f73a3c6`), and the original framing was wrong in a way worth keeping**: page padding cannot solve this at all, because `<footer>` is a *sibling* of `<main>` — every page-level reservation sits inside the element that is not being covered, which is why `/about`'s `pb-8` never helped. **The footer reserves `--space-a11y-footprint` (68px = the trigger's box + inset + air).** Verified two ways: a `StorefrontLayout` unit block across five routes (jsdom has no layout, so it pins the reservation and that no `pb-N` literal returns), and the e2e geometry loop below — now widened from the CTA alone to **every footer link**, which is what actually catches the statutory הצהרת נגישות case
