...
| **F10 storefront** — ~~shipped 2026-07-28~~; the items below are **re-homed to the post-pilot catalog-enrichment feature (alongside E10 #5)**, not dropped | No-login favorites (localStorage) feeding "dresses I want to try" into booking · 3-facet filter max, with the "הצגת 24 מתוך 57" progress label · "more like this" strip on dress pages · real-brides gallery + testimonials blocks (owner-curated) · optional English toggle (helenakolan pattern) · clean transliterated URL slugs.<br>**The load-more button itself was NOT deferred — F10 shipped it.** It was bundled with the filter in this row, but the two are different things: the filter is an enhancement, the load-more is the only path to dress 25 of the pilot's ~60. Deferring the filter is `competitors.md`'s reasoning (a filter-heavy catalog below ~40 dresses reads as inventory software, not a lookbook); deferring the load-more would have failed E2's third success criterion. |
