...
| SMC-4 | F53 | customers-crm | search/detail/notes/tags + SMS log | F31 | **done** (PR #35) |
| SMC-5 | F34 | shift-board-checkin | `checked_in_at`, check-in/undo endpoints, 5s-poll board | F15, F31 | **done** (PR #32) |
| SMC-6 | F50 | walk-in-bookings | walk-in create from board (F50's safe half; remote owner-create stays open) | F34 | queued — **unblocked**, SMC-5 merged as PR #32 |

Five of six shipped. **Re-ordered 2026-07-31**: the user self-approved SMC-5's design gate, so the board is no longer parked — it is the **first pick of the floor-management program** (`LOOP-STATE.md` → `rulings_2026_07_31`) and the shell every floor panel attaches to (F57 staff cards, F36 rooms, F58 waitlist + dispatch, F37 SOS centre). SMC-4 shipped 2026-08-03 as PR #35, built in parallel with the floor block rather than after it — the user asked for a feature depending on none of F57/F33/F19, and it was the only eligible entry whose surface was almost entirely new files. **SMC-6 (F50) is the last one open**, and it is unblocked: SMC-5 merged as PR #32. Note F50 ships from BoardSection.tsx, which F57 rewrote (the poll loop now lives in lib/usePoll.ts) — build against the merged shape, not the epic's.
