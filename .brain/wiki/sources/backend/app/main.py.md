...
blob: e4951bc29d3474bc1c0e062cc758a04fa7c60b09
...
| `DUPLICATE_EMAIL_BODY` | const | F51 — 409 on a staff email already taken within the tenant |
| `LAST_OWNER_REQUIRED_BODY` | const | F51 — 409 refusing the write that would leave a boutique with no owner. Carries **no count**: how many owners exist is not stated, because naming it would leak the tenant's staffing to a probe |
| `STAFF_SELF_MANAGE_BODY` | const | F51 — 409 on an owner changing her own role or deactivating herself |
...
**F51 mounted the fifth `/manage` router days later**, [[backend/app/auth/staff_router.py]], and parked a `StaffService` beside the existing `AuthService` rather than adding methods to it — the reason is recorded in code: `AuthService` verifies credentials and issues sessions, and folding administration into it would drag the login path's fake into every staff-CRUD test. Its three new error bodies are worth reading as a set, because all three are 409s that refuse a *write* rather than reporting a lookup, and `LAST_OWNER_REQUIRED_BODY` deliberately states no count — how many owners a boutique has is not something a probe should be able to learn by trying.
...
- [[backend/app/auth/staff_router.py]] — F51's `/manage/staff` routes
- [[backend/app/auth/staff.py]] — `StaffService` and its three error classes
