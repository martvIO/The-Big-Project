...
blob: 0de46ec6329ec1dcd4520f90ff15c10c453e8d22
...
**Role.** The public storefront read API: three anonymous, tenant-scoped `GET`s (`/storefront/dresses`, `/storefront/dresses/{dress_id}`, `/storefront/boutique`) plus the pure projection functions that map `StorefrontService`'s frozen views onto the public wire models. Rewritten in the F10 spec-conformance pass (PR #15): it now fronts its own [[backend/app/storefront/service.py]] instead of `CatalogService`, and its throttle is per-tenant with its own error class.
...
| `public_dress` / `public_dress_detail` / `public_boutique` / `public_dress_list` | fn | Module-level projections, non-underscored so tests import and assert on them directly |
